    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6 relative">
        ...
        <form id="editProductForm" method="post" action="<%= request.getContextPath() %>/product/updateProduct" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" id="titleInput" value="<%=p.getTitle()%>" required class="w-full border px-3 py-2 rounded" />
            </div>

            <!-- Description -->
            <div>
                <label class="block mb-1 font-semibold">Description</label>
                <textarea name="description" value="<%= p.getDescription() %>" id="descInput" rows="3" class="w-full border px-3 py-2 rounded"></textarea>
            </div>

            <!-- Slug -->
            <div>
                <label class="block mb-1 font-semibold">Product Slug</label>
                <input type="text" name="slug" value="<%=p.getSlug()%>" id="slugInput" readonly class="w-full border px-3 py-2 rounded" />
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="price" value="<%=p.getPrice()%>" step="0.01" required class="w-full border px-3 py-2 rounded" />
                </div>

                <div>
                    <label class="block mb-1 font-semibold">Stock</label>
                    <input type="number" value="<%=p.getStock()%>" name="stock" required class="w-full border px-3 py-2 rounded" />
                </div>
            </div>
            ...
            <div>
                ...
                <select name="typeId" required class="w-full border px-3 py-2 rounded">
                    ...
                    if (types != null) {
                        // First display the currently selected category
                        for (productType t : types) {
                            if (t.getId() == p.getTypeId().getId()) {
                    %>
                    ...
                    <%
                        ...
                        // Then display all other categories
                        for (productType t : types) {
                            if (t.getId() != p.getTypeId().getId()) {
                    %>
            ...
            <div>
                ...
                <input type="text" name="retailInfo" value="<%=p.getRetailInfo()%>" class="w-full border px-3 py-2 rounded" />
            </div>
