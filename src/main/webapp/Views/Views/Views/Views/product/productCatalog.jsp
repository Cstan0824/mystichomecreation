...
<body class="bg-gray-50">
    ...
    <script>
           ...
           async function filterByCategory() {
                const selected = [...document.querySelectorAll('input[name="filterCategory[]"]:checked')]
                    .map(cb => 'categories[]='+encodeURIComponent(cb.value)+'').join('&');

                try {
                    const res = await fetch(`/product/productCatalog/Filter?${selected}`);
